...
(eval-when-compile
  (defvar user-ext-sh-function-regex))
(defun sh-ext--looking-at (what &optional position)
  "Return t if text point matches a pattern for form WHAT.
WHAT is a symbol indicating the form we want to match; it
can be one of the following symbols listed below.
...
(sh-ext-define-skeleton while-getops
  ...
  ( (let ((opt (and v1 (char-to-string (car v1))))
	  (flag (nth 1 v1)))
      (prog1 opt
	(if (eql flag ?:)
	    (setq v1 (nthcdr 2 v1)
