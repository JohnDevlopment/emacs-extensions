...
(defgroup user-ext-liquidsoap nil
...
(defcustom user-ext-liquidsoap-binary-path ""
...
(defvar user-ext-liquidsoap-doc-cache nil
  "An alist of commands mapped to their docstrings.")

(defconst user-ext-liquidsoap-docheader-regexp
...
cache, this returns nil."
  (cl-assert (listp user-ext-liquidsoap-doc-cache))
  (assoc command user-ext-liquidsoap-doc-cache #'string=))
...
This function modifies `liquidsoap-ext-doc-cache'."
  (cl-assert (listp user-ext-liquidsoap-doc-cache))
  (cl-pushnew (cons command doc) user-ext-liquidsoap-doc-cache))
...
(defun liquidsoap-ext--get-doc (command)
  ...
  (let ((doc (liquidsoap-ext--cache-get-doc command))
    ...
    (unless doc
      (call-process user-ext-liquidsoap-binary-path	  ; Call subprocess to get documentation
	...
	(cl-assert (stringp result)))	      ; Make sure result is a string
...
BUFFER is a buffer containing Liquidsoap documentation."
  (let (line pm beg end)
	...
	(while (not (eobp))
	  ...
	  (if (looking-at user-ext-liquidsoap-docheader-regexp)
	      ;; Header, make it bold
...

(extension-provide 'liquidsoap-ext)
