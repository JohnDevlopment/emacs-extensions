  ...
  (if-let ((conv (commit-helper-alist-get
	...
	(concat (if wip "WIP: " "")
		(apply
		 ...
		 (or (and context
			  (not (string= context "-"))
			  (let ((fn (commit-helper-convention-context-transform conv)))
			    (list tag (funcall fn context) message)))
		     (list tag message)))))
...
(global-set-key (kbd "C-c c c") #'commit-helper)
