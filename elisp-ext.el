...
(defvar user-ext-elisp--motion-transient-map (make-sparse-keymap "Motion: "))


;; --- Occur

(define-obsolete-variable-alias 'user-ext-elisp-valid-occur-types
  'user-ext-elisp-occur-valid-types "2025-09-18")
(defconst user-ext-elisp-occur-valid-types
  '(functions sections types variables))

(defconst user-ext-elisp-occur-type-alist
  (alist-ext-define "cl-defgeneric" "Generic Function"
		    "cl-defmacro"   "Macro"
		    "cl-defsubst"   "Inline Function"
		    "defmacro"      "Macro"
		    "defsubst"      "Inline Function"))

(defconst user-ext-elisp-occur-defun-regexp
  (elisp-ext--rx
   ?\( (group (or "cl-defun"
		  "cl-defgeneric"
		  "cl-defmacro"
...
(defconst user-ext-elisp-occur-variable-regexp
  (elisp-ext--rx
...
(defsubst elisp-ext--set-transient-map ()
  (set-transient-map user-ext-elisp--motion-transient-map))
...
;;;###autoload
(defun elisp-ext-update-loadefs (&optional regenerate interactive-p)
...
`user-ext-extension-directory' and \"loaddefs-ext.el\"."
  ...
  (let ((generated-autoload-file
	 (f-join user-ext-extension-directory "loaddefs-ext.el")))
    (cl-assert (stringp generated-autoload-file) t)
    (emacs-version-cond
      ((>= "29.1")
       (loaddefs-generate
	(list "~/.emacs.d/extensions/" "~/.emacs.d/extensions/packages/")
	generated-autoload-file
	nil
	nil
	nil
	regenerate))
      (t (update-directory-autoloads
	  "~/.emacs.d/extensions/"
...
(defun elisp-ext-set-mark-command (arg)
  "Set the mark where point is and activate it, or jump to the mark.
This is a \"cycling\" variation of `set-mark-command' (see below).

This is a \"cycling\" command in the sense that it can be
repeated using the same key without its prefix.  For the
duration of the next command, the following bindings apply:

\\{user-ext-elisp--motion-transient-map}"
...
(cl-defmethod elisp-ext--list-x ((_type (eql functions)))
  ...
  (elisp-ext--search-regexp
   user-ext-elisp-occur-defun-regexp
   (lambda ()
     (let (sym name keyword type)
       (setq keyword (match-string-no-properties 1)
	     ...
	     type (if sym
			...
			(_ (alist-get keyword
				      user-ext-elisp-occur-type-alist
				      "???" nil #'string=)))
		    "Undefined"))
...
(cl-defmethod elisp-ext--list-x ((_type (eql variables)))
  ...
  (let (v1 v2)
    (setq v1 (elisp-ext--search-regexp
	      user-ext-elisp-occur-variable-regexp
	  ...
	  v2 (elisp-ext--search-regexp
	      user-ext-elisp-occur-mode-local-variable-regexp
      ...
      (cl-loop while (re-search-forward rx nil t) ; Searching for (sub)section comments
		...
		((looking-at user-ext-elisp-subsection-comment-regexp)
		 ...
		 (push (propertize temp
				   'length (length temp)
				   'position (unless
						 (elisp-ext--occur-separator-string
						  temp)
					       (match-beginning 0)))
		       ssl)))
	       (forward-line)
...
(defun elisp-ext--occur-buttonize-entries (type)
  ...
  (save-excursion
	...
	(cl-loop with start
		 ...
		 while (not (eobp))
		 do
		 (ignore (buffer-substring (point) (1+ (point))))
		 (setq start (point)
		       end (let ((sz (get-text-property (point) 'length)))
			     (cl-check-type sz integer)
			     (+ start sz)))
		 (unless (elisp-ext--occur-separator-string
...
(fext-defadvice tabulated-list-sort (after tabulated-list-sort)
  ...
  (when (eq major-mode 'elisp-occur-mode)
    (run-with-idle-timer 0.01 nil
			 (lambda ()
			   (elisp-ext--occur-buttonize-entries elisp-occur-type)))))
...
With ARG, do it that many times.  With a negative ARG,
reverse the direction of motion.
If point is on an opening parenthesis, go to the position
immediately after the matching closing parenthesis.
If point is right after an opening parenthesis, go the
position of the matching closing parenthesis.
If point is on or right after a closing parenthesis, go to
the position of the matching opening parenthesis.

When called interactively, ARG is the numeric prefix argument.
...
\\{user-ext-elisp--motion-transient-map}"
  (interactive "^p")
  (cl-ext-cond
    ;; Point is at opening parenthesis
    ;; (looking-at "\\s(")
    ;; ^               =>|
    ((looking-at "\\s(")
     (forward-sexp arg))
    ;; Point is at closing parenthesis
    ;; (looking-at "\\s)")
    ;; |<=               ^
    ((looking-at "\\s)")
     (forward-char)
     (backward-sexp arg))
    ;; Point is right after closing parenthesis
    ...
    ((looking-back "\\s(" (1- (point)))
     ...
     (backward-char))
    (t (cl-ext-save-point
	 (cl-block end-of-buffer
	   (if (= arg 1)
	       (cl-ext-progn
		 (skip-syntax-forward "^()")
		 (or (eobp) (setq user-ext-cl--point (point-marker))))
	     (dotimes (_i arg)
	       (skip-syntax-forward "^(")
	       (and (eobp) (cl-return-from end-of-buffer)))
	     (setq user-ext-cl--point (point-marker)))))))
  (elisp-ext--set-transient-map))

(defun elisp-ext-jump-sexp-level (&optional n)
  "Jump N levels up the S-expression around point.
Interactively, N is the numerical prefix argument.
  ...
  (let* ((pps (make-ppss-easy (syntax-ppss)))
	 (pos (cl-ext-progn
		...
		(if n
		  ...
		  (ppss-innermost-start pps)))))
    (or pos (user-error "Not inside a Lisp expression"))
    (goto-char pos)
    (if (region-active-p)
	(cl-ext-progn
	  (elisp-ext-forward-or-backward-sexp)
	  (push-mark pos))
      (elisp-ext--set-transient-map))))

(defun elisp-ext-left-char (&optional n)
  "Move point N characters to the left (or right if N is negative).
This is a \"cycling\" variation of `left-char', which see.
...
\\{user-ext-elisp--motion-transient-map}"
  (declare (interactive-only left-char))
  (interactive "p")
  (unwind-protect
      (left-char n)
    (elisp-ext--set-transient-map)))

(defun elisp-ext-right-char (&optional n)
  "Move point N characters to the right (or left if N is negative).
This is a \"cycling\" variation of `right-char', which see.
...
\\{user-ext-elisp--motion-transient-map}"
  (declare (interactive-only right-char))
  (interactive "p")
  (unwind-protect
      (right-char n)
    (elisp-ext--set-transient-map)))
...
(defun elisp-ext--scratch-buffer-ctrl-c-ctrl-c ()
  ...
  (let ((inhibit-read-only t))
	...
	(replace-match ""))		   ; from the end of the buffer
      (when user-ext-elisp--scratch-minify ; Minify the buffer with prefix argument
...
(defun elisp-ext-doc-scratch-buffer--shift-return (&optional arg)
  "Insert a newline and change fill column.
If ARG is non-nil, or when called interactively with a
prefix argument, do not insert a newline."
  (interactive "P")
  (or arg (newline))
  (set-fill-column 60))
...
it is restored."
  (interactive "*")
  (when-let ((bounds (thing-at-point-ext-bounds-of-string-at-point))
	     (beg (1+ (car bounds)))
	     (end (1- (cdr bounds)))
	     (string (if (= beg end) ""
    ...
    (unless (= beg end)
      (kill-region beg end)
      (run-with-idle-timer
       1 nil #'message "Old documentation string added to kill ring"))
...
\\{elisp-ext-doc-minor-mode-map}"
  :keymap (let ((map (make-sparse-keymap)))
	    ...
	    (define-key map (kbd "M-F") #'fill-region)
...
error."
  (cl-ext-progn
    ...
    (save-excursion
      (and (comment-search-forward (line-end-position) noerror)
	   (current-column)))))
...
(eval-and-compile
  ...
  (define-key emacs-lisp-mode-map (kbd "<mouse-3>") #'imenu)
  ...
  (define-key emacs-lisp-mode-map (kbd "C-c c n") #'emacs-lisp-native-compile)
  (define-key emacs-lisp-mode-map (kbd "C-c c M-n") #'emacs-lisp-native-compile-and-load)
  ...
  (define-key emacs-lisp-mode-map (kbd "C-@") #'elisp-ext-set-mark-command)

  ;; Transient map for motion commands
  (define-key user-ext-elisp--motion-transient-map (kbd "^") #'elisp-ext-jump-sexp-level)
  (define-key user-ext-elisp--motion-transient-map (kbd "p") #'elisp-ext-forward-or-backward-sexp)
  (define-key user-ext-elisp--motion-transient-map (kbd ",") #'elisp-ext-left-char)
  (define-key user-ext-elisp--motion-transient-map (kbd ".") #'elisp-ext-right-char)
  (define-key user-ext-elisp--motion-transient-map (kbd "@") #'elisp-ext-set-mark-command)
  ...
  (define-key lisp-interaction-mode-map (kbd "C-c c n") #'emacs-lisp-native-compile)
  (define-key lisp-interaction-mode-map (kbd "C-c c M-n") #'emacs-lisp-native-compile-and-load)
...

(extension-provide 'elisp-ext)
;;; elisp-ext ends here

;; Local Variables:
;; eval: (abbrev-ext-install-local-abbrev-functions)
;; eval: (abbrev-ext-define-local-abbrev "ex" "elisp-ext")
;; eval: (abbrev-ext-define-local-abbrev "ux" "user-ext-elisp")
;; End:
