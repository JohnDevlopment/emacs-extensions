...
(defvar-local local-lambda--lambda-history nil)
...
(defun local-lambda--completion ()
  (let ((hash local-lambda-lambdas)
	key)
    (or hash (user-error "No local lambdas in buffer"))
    (setq key (completing-read
	       "Lambda: " hash nil t nil 'local-lambda--lambda-history))
    (cl-assert key t)
    (list key)))
(define-obsolete-function-alias
...
Interactively, KEY is prompted from the user with completion."
  ...
  (let ((fun (local-lambda-get key)))
    (or fun (error "No function under key %S" key))
    (if (commandp fun)
...
(extension-provide 'local-lambda)
